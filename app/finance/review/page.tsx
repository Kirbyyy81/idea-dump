                        ...
                        <form onSubmit={(event) => void resolveItem('confirm', event)}>
                            <Card className="p-5">
                                ...
                                <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                                    <Button type="button" variant="ghost" icon={<CloseDoodleIcon size={15} />} onClick={() => void resolveItem('reject')} disabled={isSaving}>Reject as not a transaction</Button>
                                    {selected.payload.duplicate_transaction_id && <Button type="button" variant="secondary" onClick={() => void resolveItem('mark_duplicate')} disabled={isSaving}>Mark duplicate</Button>}
                                    <Button type="submit" icon={<CheckDoodleIcon size={15} />} isLoading={isSaving} disabled={isDateProposalPending || !form.source_id || (form.source_id === NEW_SOURCE && !newSourceName.trim()) || (form.category_id === NEW_CATEGORY && !newCategoryName.trim()) || (duplicateOutcome(selected) !== 'none' && !form.allow_duplicate) || (duplicateOutcome(selected) === 'strong' && !form.duplicate_override_reason.trim())}>Confirm transaction</Button>
                                </div>
                            </Card>
                        </form>
