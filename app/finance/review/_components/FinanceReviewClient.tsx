                            ...
                            <Card className="p-5">
                                ...
                                {duplicateOutcome(selected) !== 'none' && (
                                    <div className="mt-4 border border-warning bg-warning-bg px-4 py-3 text-sm text-warning">
                                        ...
                                        <ToggleField id="review-allow-duplicate" toggleLabel="Confirm anyway" containerClassName="mt-3" errorMessage={fieldErrors.allow_duplicate} data-finance-field="allow_duplicate" checked={form.allow_duplicate} onChange={(allow_duplicate) => setReviewField('allow_duplicate', allow_duplicate)} />
                                        {form.allow_duplicate && <TextareaField id="review-duplicate-reason" label="Override reason" containerClassName="mt-3" required={duplicateOutcome(selected) === 'strong'} errorMessage={fieldErrors.duplicate_override_reason} data-finance-field="duplicate_override_reason" maxLength={500} value={form.duplicate_override_reason} onChange={(event) => setReviewField('duplicate_override_reason', event.target.value)} placeholder="Why is this a separate transaction?" />}
                                    </div>
                                )}
                                ...
                                <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
                                    <div>
                                        <SelectField
                                            id="review-direction"
                                            label="Direction"
                                            data-finance-field="direction"
                                            errorMessage={fieldErrors.direction}
                                            aria-label="Transaction direction"
                                            aria-describedby={isDirectionProposalPending ? 'direction-proposal-help' : undefined}
                                    ...
                                    <InputField id="review-amount" label="Amount" required errorMessage={fieldErrors.amount} data-finance-field="amount" type="number" min="0.01" max={MAX_FINANCE_AMOUNT} step="0.01" value={form.amount} onChange={(event) => setReviewField('amount', event.target.value)} />
                                    <InputField id="review-currency" label="Currency" value="MYR" readOnly aria-readonly="true" />
                                    <InputField id="review-reference" label="Transaction reference" containerClassName="md:col-span-2" errorMessage={fieldErrors.reference_number} data-finance-field="reference_number" maxLength={MAX_FINANCE_REFERENCE_LENGTH} value={form.reference_number} onChange={(event) => setReviewField('reference_number', event.target.value)} />
                                    <div className="space-y-4"><SelectField id="review-source" label="Source" required errorMessage={fieldErrors.source_id} data-finance-field="source_id" disabled={referenceStatus !== 'ready'} aria-label="Transaction source" value={form.source_id} onChange={(source_id) => setReviewField('source_id', source_id)} placeholder="Choose a source" options={[...sources.map((source) => ({ value: source.id, label: source.name })), { value: NEW_SOURCE, label: '+ Add new source' }]} />{form.source_id === NEW_SOURCE && <InputField id="review-new-source" label="New source name" required errorMessage={fieldErrors.new_source_name} data-finance-field="new_source_name" maxLength={MAX_FINANCE_NAME_LENGTH} value={newSourceName} onChange={(event) => { setNewSourceName(event.target.value); setFieldErrors((current) => ({ ...current, new_source_name: undefined })); }} placeholder="e.g. Maybank" />}</div>
                                    <div className="space-y-4"><SelectField id="review-category" label="Category" errorMessage={fieldErrors.category_id} data-finance-field="category_id" disabled={referenceStatus !== 'ready'} aria-label="Transaction category" value={form.category_id} onChange={(category_id) => setReviewField('category_id', category_id)} placeholder="Uncategorised" options={[{ value: '', label: 'Uncategorised' }, ...availableCategories, { value: NEW_CATEGORY, label: '+ Add new category' }]} />{form.category_id === NEW_CATEGORY && <InputField id="review-new-category" label="New category name" required errorMessage={fieldErrors.new_category_name} data-finance-field="new_category_name" maxLength={MAX_FINANCE_NAME_LENGTH} value={newCategoryName} onChange={(event) => { setNewCategoryName(event.target.value); setFieldErrors((current) => ({ ...current, new_category_name: undefined })); }} placeholder="e.g. Groceries" />}</div>
                                    <InputField id="review-merchant" label="Merchant (optional)" errorMessage={fieldErrors.merchant} data-finance-field="merchant" maxLength={MAX_FINANCE_MERCHANT_LENGTH} value={form.merchant} onChange={(event) => setReviewField('merchant', event.target.value)} />
                                    <ToggleField id="review-has-payee" label="Payee" toggleLabel="Is a payee" errorMessage={fieldErrors.has_payee} data-finance-field="has_payee" checked={form.has_payee} onChange={setReviewPayeeClassification} aria-label="Is a payee" />
                                    {form.has_payee && <InputField id="review-payee" label="Payee" required errorMessage={fieldErrors.payee_name} data-finance-field="payee_name" maxLength={MAX_FINANCE_PAYEE_LENGTH} value={form.payee_name} onChange={(event) => setReviewField('payee_name', event.target.value)} />}
                                    <div><InputField id="review-date" label="Date" required errorMessage={fieldErrors.transaction_date} data-finance-field="transaction_date" type="date" max={today} aria-describedby={isDateProposalPending ? 'date-proposal-help' : undefined} value={form.transaction_date} onChange={(event) => { setReviewField('transaction_date', event.target.value); setIsDateProposalPending(false); }} />{isDateProposalPending && <span id="date-proposal-help" className="mt-1 block text-xs text-warning">No date was detected. Today is proposed; confirm this date before continuing.</span>}{isDateProposalPending && <Button type="button" variant="ghost" onClick={() => { setIsDateProposalPending(false); setFieldErrors((current) => ({ ...current, transaction_date: undefined })); }}>Use proposed date</Button>}</div>
                                    <TextareaField id="review-notes" label="Notes" containerClassName="md:col-span-2" errorMessage={fieldErrors.notes} data-finance-field="notes" maxLength={MAX_FINANCE_NOTES_LENGTH} value={form.notes} onChange={(event) => setReviewField('notes', event.target.value)} />
                                </div>
