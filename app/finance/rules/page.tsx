...
import { FinanceAccount, FinanceCategory, FinanceRule, FinanceRuleSuggestion, FinanceTransactionDirection } from '@/lib/types';
import { useAlert } from '@/lib/contexts/AlertContext';

type MatchType = FinanceRule['match_type'];
type RuleWithRelations = FinanceRule & { account?: FinanceAccount | null; category?: FinanceCategory | null };
...
const matchTypeOptions = [
    ...
    { value: 'account_hint', label: 'Account hint' },
];
...
export default function FinanceRulesPage() {
    ...
    const loadData = useCallback(async () => {
        try {
            const [rulesResponse, accountsResponse, categoriesResponse, suggestionsResponse] = await Promise.all([
                fetch('/api/finance/rules'), fetch('/api/finance/accounts'), fetch('/api/finance/categories'), fetch('/api/finance/rule-suggestions'),
            ]);
            const [rulesPayload, accountsPayload, categoriesPayload, suggestionsPayload] = await Promise.all([
                rulesResponse.json(), accountsResponse.json(), categoriesResponse.json(), suggestionsResponse.json(),
            ]);
    ...
    return (
        <AppShell contentClassName="p-5 md:p-8">
            <div className="mx-auto max-w-7xl">
                ...
                <FinanceNav currentPath="/finance/rules" />
                    ...
                    <form onSubmit={addRule}>
                        <Card className="p-5">
                            ...
                            <div className="mt-5 space-y-4">
                                ...
                                <label className="block space-y-2"><span className="text-sm text-text-secondary">Set account</span><Select value={form.account_id} onChange={(account_id) => setForm({ ...form, account_id })} options={[{ value: '', label: 'Do not change' }, ...accounts.filter((account) => !account.is_archived).map((account) => ({ value: account.id, label: account.name }))]} /></label>
                                ...
                                <div key={rule.id} className="px-5 py-4">
                                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                        <div className="min-w-0">
                                            ...
                                            <p className="mt-2 text-sm text-text-secondary">{[rule.account?.name, rule.category?.name, rule.direction].filter(Boolean).join(' - ')} - Priority {rule.priority}</p>
                                        </div>
