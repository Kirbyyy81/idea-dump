...
export function FinanceActivityCalendar({ month, today, items, selectedDate: controlledDate, onSelectDate, pending = false }: {
    ...
    const [localDate, setLocalDate] = useState(today.startsWith(`${month}-`) ? today : latestActivity ?? firstDay);
    const selectedDate = controlledDate ?? localDate;
            ...
            <div className="min-w-[280px]">
                ...
                <div className="grid grid-cols-7" role="group" aria-label="Daily activity calendar">
                    ...
                    {days.map((date, index) => {
                        ...
                        const selectedDay = date === (controlledDate === undefined ? localDate : controlledDate);
                        return <Button key={date} type="button" variant="ghost" disabled={future || pending}
                            ...
                            onClick={() => { setLocalDate(date); onSelectDate?.(date); }}
