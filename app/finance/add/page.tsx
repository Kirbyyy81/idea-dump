...
export default function AddFinanceTransactionPage() {
    ...
    return <AppShell contentClassName="p-5 md:p-8"><div className="mx-auto max-w-2xl">
        ...
        </form> : <form onSubmit={submitScreenshot} className="mt-6"><FileUpload label="Transaction screenshot" accept="image/png,image/jpeg,image/webp" value={file} onChange={setFile} /><p className="mt-2 text-sm text-text-muted">PNG, JPEG, or WebP up to 4 MB. Compress larger images before uploading. The image is processed synchronously and is not retained.</p><Button type="submit" className="mt-5 w-full" isLoading={isSaving} disabled={!file}>Process screenshot</Button><p className="mt-4 text-center text-sm text-text-muted">One candidate is created per screenshot. Items OCR cannot confirm will appear in the <Link href="/finance/review" className="font-semibold text-accent-blue hover:underline">review queue</Link>.</p></form>}
    </div></AppShell>;
}
