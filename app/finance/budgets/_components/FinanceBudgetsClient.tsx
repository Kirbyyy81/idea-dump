...
export function FinanceBudgetsClient({ initialList, initialState, initialDetail }: {
    ...
    const load = async (nextState = state, page = list.page, id?: string, historyPage = 1, transactionsPage = 1, focus = false) => {
        ...
        try {
            const nextList = await financeApiRequest<FinanceBudgetPage<FinanceBudgetSummary>>(`/api/finance/budgets?state=${nextState}&page=${page}`);
            const selected = id ?? nextList.data[0]?.id;
            const nextDetail = selected ? (await financeApiRequest<{ data: FinanceBudgetDetail }>(`/api/finance/budgets/${selected}?history_page=${historyPage}&transactions_page=${transactionsPage}`)).data : null;
            if (sequence !== requestSequence.current) return;
            setState(nextState); setList(nextList); setDetail(nextDetail);
            const params = new URLSearchParams({ state: nextState, page: String(page) });
            if (selected) params.set('budget', selected);
