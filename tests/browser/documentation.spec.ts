...
test('loads readable content before offscreen tables, retries locally, and searches all remaining content', async ({ page }, testInfo) => {
    ...
    await page.route(`**/api/documentation/${id}/content?*`, async (route) => {
        ...
        if (parent === table) {
            ...
            toggleReads++;
            await route.fulfill({ json: { data: { blocks: [block('hidden', 'paragraph', 'needle inside toggle')], nextCursor: null } } });
        } else {
            rootReads++;
            if (!query.has('cursor')) await new Promise<void>((resolve) => { releaseRoot = resolve; });
            await route.fulfill({ json: { data: query.has('cursor')
                ? { blocks: [block('last', 'paragraph', 'needle on final page')], nextCursor: null }
