...
- `lib/finance/` is the Finance feature root. Keep concise Finance capabilities at this level, including catalog, rules, review, dashboard, authorization, shared values, and cross-capability orchestration.
...
- Keep browser-only Finance code in `client.ts`, `catalogClient.ts`, or `share/client.ts`, each marked with `'use client'` where browser APIs are used.
...
- Keep service-role queries in Finance repositories or reviewed server-side helpers, with explicit verified-user filters.
