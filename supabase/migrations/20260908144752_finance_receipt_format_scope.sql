-- Additive rollout: deploy compatible readers before enabling Ryt format detection.
alter table public.finance_intake_items
 add column receipt_format_eligible boolean not null default false,
 add column receipt_format text not null default 'unknown' check(receipt_format in ('unknown','ryt_screenshot_v1','ryt_shared_v1')),
 add column receipt_detector_version integer check(receipt_detector_version=1),
 add column receipt_processing jsonb;
-- Existing rows retain false, including failed or pending historical uploads.
alter table public.finance_intake_items alter column receipt_format_eligible set default true;
alter table public.finance_parser_templates add column scope_receipt_format text
 check(scope_receipt_format in ('ryt_screenshot_v1','ryt_shared_v1')),
 add constraint finance_template_format_field check(scope_receipt_format is null or (field_name<>'source_id' and algorithm_version=2));

create function finance_private.finance_guard_receipt_format() returns trigger
language plpgsql security invoker set search_path='' as $fn$
begin
 if tg_op='UPDATE' and new.scope_receipt_format is distinct from old.scope_receipt_format then
  raise exception using errcode='23514',message='Template receipt format is immutable'; end if;
 if new.scope_receipt_format is not null and not exists(select 1 from public.dim_finance_sources s
   where s.id=new.scope_source_id and s.user_id=new.user_id and public.finance_template_source_phrase_v2(s.name)='ryt bank') then
  raise exception using errcode='23514',message='Ryt receipt scope requires an owned Ryt source'; end if;
 return new;
end; $fn$;
create trigger finance_guard_receipt_format before insert or update on public.finance_parser_templates
 for each row execute function finance_private.finance_guard_receipt_format();

create function public.finance_receipt_format_matches(p_scope text,p_format text) returns boolean
language sql immutable security invoker set search_path='' as $fn$
 select case when p_scope is null then coalesce(p_format,'unknown')<>'ryt_shared_v1' else p_scope=p_format end;
$fn$;
create function public.finance_finalize_screenshot_intake_v3(
  p_user_id uuid,
  p_intake_id uuid,
  ...
  p_receipt_processing jsonb
)
returns jsonb language plpgsql security invoker set search_path='' as $fn$
declare item public.finance_intake_items%rowtype; result jsonb; fmt text:=p_receipt_processing->>'format';
begin
 ...
 if p_receipt_processing is null or jsonb_typeof(p_receipt_processing)<>'object'
  or not (p_receipt_processing ?& array['format','detector_version','failed_regions','conflicts'])
  or p_receipt_processing - array['format','detector_version','failed_regions','conflicts'] <> '{}'::jsonb
  or fmt is null or fmt not in ('unknown','ryt_screenshot_v1','ryt_shared_v1')
  or p_receipt_processing->'detector_version' is distinct from '1'::jsonb
  or jsonb_typeof(p_receipt_processing->'failed_regions') is distinct from 'array'
  or jsonb_typeof(p_receipt_processing->'conflicts') is distinct from 'array'
  or octet_length(p_receipt_processing::text)>2048 then raise exception using errcode='22023',message='Invalid receipt diagnostics'; end if;
 if jsonb_array_length(p_receipt_processing->'failed_regions')>3 or jsonb_array_length(p_receipt_processing->'conflicts')>6
  or exists(select 1 from jsonb_array_elements_text(p_receipt_processing->'failed_regions') x(v) where v not in ('header','recipient','details') or v is null)
  or exists(select 1 from jsonb_array_elements_text(p_receipt_processing->'conflicts') x(v) where v not in ('amount','transaction_date','payee_name','reference_number','direction','notes') or v is null)
  then raise exception using errcode='22023',message='Invalid receipt diagnostic fields'; end if;
 select * into item from public.finance_intake_items where id=p_intake_id and user_id=p_user_id for update;
 if not found then raise exception using errcode='P0002',message='Finance intake not found'; end if;
 if not exists(select 1 from public.finance_candidate_transactions c where c.intake_item_id=item.id and c.user_id=p_user_id) then
 if fmt<>'unknown' and not item.receipt_format_eligible then
  raise exception using errcode='23514',message='Historical intake cannot acquire a receipt format'; end if;
 if fmt<>'unknown' and not exists(select 1 from public.dim_finance_sources s where s.id=p_detected_source_id
   and s.user_id=p_user_id and not s.is_archived and lower(trim(regexp_replace(s.name,'[^a-zA-Z0-9]+',' ','g')))='ryt bank') then
  raise exception using errcode='23514',message='Receipt format source mismatch'; end if;
  if item.status<>'processing' or item.processing_attempt_id is distinct from p_processing_attempt_id then
   raise exception using errcode='40001',message='Finance processing attempt no longer owns this intake'; end if;
  update public.finance_intake_items set receipt_format=fmt,receipt_detector_version=1,receipt_processing=p_receipt_processing where id=item.id;
 end if;
 result:=public.finance_finalize_screenshot_intake_v2(p_user_id,p_intake_id,p_processing_attempt_id,p_ocr_raw_text,p_ocr_normalized_text,p_ocr_confidence,p_ocr_text_hash,p_normalizer_version,p_detected_source_id,p_source_detection_signals,p_candidate_payload || jsonb_build_object('receipt_processing',p_receipt_processing),p_candidate_confidence,p_matched_rule_id,p_duplicate_outcome,p_duplicate_score,p_duplicate_signals,p_duplicate_explanation,p_duplicate_transaction_id);
 return result;
end; $fn$;
revoke all on function public.finance_finalize_screenshot_intake_v3(uuid,uuid,uuid,text,text,numeric,text,integer,uuid,jsonb,jsonb,numeric,uuid,text,numeric,jsonb,text,uuid,jsonb) from public,anon,authenticated;
grant execute on function public.finance_finalize_screenshot_intake_v3(uuid,uuid,uuid,text,text,numeric,text,integer,uuid,jsonb,jsonb,numeric,uuid,text,numeric,jsonb,text,uuid,jsonb) to service_role;
...
revoke all on function finance_private.finance_guard_receipt_format(),public.finance_receipt_format_matches(text,text) from public,anon,authenticated,service_role;
