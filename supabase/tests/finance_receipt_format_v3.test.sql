...
begin
 ...
 values(intake,'b4000000-0000-4000-8000-000000000001','screenshot','completed','2026-09-09T00:00:00Z','Ryt Bank.png','Receipt',fmt);
 ...
 jsonb_build_object('parser_template_baseline',jsonb_build_object('reference_number','OCR-SYN-'||n)) || case when observed is null then '{}'::jsonb else jsonb_build_object('parser_template_evaluations',jsonb_build_array(jsonb_build_object(
 'template_id',observed,'algorithm_version',3,'template_version',1,'status','shadow','outcome','shadow','value_hash',public.finance_template_value_hash_v2('reference_number','SYN-'||n)))) end);
...
select public.finance_refresh_rule_suggestions();
select pg_temp.check_format(not exists(select 1 from public.finance_parser_templates where field_name='reference_number' and algorithm_version<>3),'only algorithm 3 transforms are proposed');
select pg_temp.check_format((select count(*)=2 from public.finance_parser_templates where field_name='reference_number'),'algorithm 3 configurations have separate identities per format');
select pg_temp.check_format((select bool_and(status='shadow' and evidence_count=3 and contradiction_count=0) from public.finance_parser_templates where field_name='reference_number'),'each format enters shadow using only its three observations');
...
do $immutability$ begin
 begin
  update public.finance_parser_templates set scope_receipt_format=null where field_name='reference_number';
...
select public.finance_refresh_rule_suggestions();
select pg_temp.check_format((select status='shadow' and contradiction_count=0 from public.finance_parser_templates where field_name='reference_number' and scope_receipt_format='ryt_shared_v1'),'screenshot contradiction does not contaminate shared rule');
select pg_temp.check_format((select status='rejected' and contradiction_count=1 from public.finance_parser_templates where field_name='reference_number' and scope_receipt_format='ryt_screenshot_v1'),'matching screenshot contradiction rejects its rule');
-- Fresh post-shadow shared reviews can promote only the shared rule.
select pg_temp.format_fixture(n,'ryt_shared_v1',(select id from public.finance_parser_templates where field_name='reference_number' and scope_receipt_format='ryt_shared_v1')) from generate_series(10,12) n;
select public.finance_refresh_rule_suggestions();
select pg_temp.check_format(public.finance_parser_template_can_promote_v2((select id from public.finance_parser_templates where field_name='reference_number' and scope_receipt_format='ryt_shared_v1')),'three fresh matching observations permit promotion');
-- Even incorrectly attached evidence must fail the promotion gate.
update public.finance_template_evidence set intake_item_id=md5('format-intake-1')::uuid where candidate_id=md5('format-candidate-10')::uuid and template_id=(select id from public.finance_parser_templates where field_name='reference_number' and scope_receipt_format='ryt_shared_v1');
select pg_temp.check_format(not public.finance_parser_template_can_promote_v2((select id from public.finance_parser_templates where field_name='reference_number' and scope_receipt_format='ryt_shared_v1')),'unknown intake evidence blocks promotion even when attached incorrectly');
select public.finance_refresh_rule_suggestions();
...
select public.finance_refresh_rule_suggestions();
select pg_temp.check_format((select status='rejected' and contradiction_count>0 from public.finance_parser_templates where field_name='reference_number' and scope_receipt_format='ryt_shared_v1'),'OCR conflict blocks field promotion');
