...
- **Status:** Implemented for sideload testing; backend rollout and physical acceptance pending
- **Owner:** Ashley
...
| Project location | Use the `android/` Kotlin/Compose subproject with its pinned Gradle wrapper, JDK 17, and Android SDK 36. |
...
- **Data:** Extend user-owned Finance intake and candidate lineage with paired devices and bounded notification events. Delete raw server text at final review disposition; retain local payloads until acceptance or explicit discard, with no age-based eviction.
- **Interfaces:** The companion notification, pairing, source-list, and device-management endpoints below, the Android local queue, Spotify playback observation, LRCLIB lookups, and Android Auto metadata publication.
- **Idempotency:** Persist `client_event_id` once per captured event; enforce replay and conflict handling per authenticated user. Use package, notification key, and original post timestamp as the stable local identity. A changed post timestamp is a new event; existing Finance duplicate assessment still applies.

### Repository integration baseline

Historical baseline checked on 2026-09-23, before this implementation:

- The baseline Finance module documented manual entry and screenshot OCR, including Android PWA share batches. The baseline had no Android subproject or notification ingestion route.
...
- `FinanceTransactionSource` and the source validator in [`lib/finance/core/auth.ts`](../../lib/finance/core/auth.ts) allowed only `manual` and `screenshot`. This implementation adds `notification` ledger lineage while preserving the existing confirmation workflow.
...
The implementation contracts below supersede the conceptual entity and route names in the supplied draft. The canonical migrations and [rollout guide](../ANDROID_COMPANION.md) define the persisted schema and API limits.
...
The canonical migrations add the following server-owned entities with RLS and service-only access.
...
Pairing requests live in `companion_pairing_requests`; per-account lookup/approval allowances live in `companion_pairing_attempts`.
...
```text
POST /api/companion/notifications
```
...
POST   /api/companion/pair/approve
POST   /api/companion/pair/complete
GET    /api/companion/finance-sources
DELETE /api/companion/devices/:id
GET    /api/companion/devices
```

Pairing codes expire after five minutes. The phone generates separate proof and credential secrets before exchange; the server stores only hashes. The device credential remains valid until revoked or Finance access is removed. Reconnect replaces the local credential. There is no general-purpose refresh route.
...
Authentication must use the scoped, revocable device credential established through browser-based pairing. Device requests use the narrow bearer credential; browser approval and management retain Finance session and origin checks.

Example request with fictional data:

```json
{
  "client_event_id": "20000000-0000-4000-8000-000000000001",
  "source_id": "20000000-0000-4000-8000-000000000002",
  "notification_key_hash": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "captured_at": "2026-09-23T15:05:00+08:00",
  "source_package": "my.com.tngdigital.ewallet",
  "notification": {
    "title": "TNG eWallet",
    "text": "Alex has transferred RM 25.90 to you. Tap here to check the transaction details",
    "subtext": null,
...
The server bounds the JSON body and text fields, validates timestamps and owned sources, and rejects packages outside the enabled allowlist. Ryt/TNG use the supplied wording fixtures; UOB remains disabled.
...
- **Authentication:** Lyrics does not require Finance pairing. Finance uses a scoped, revocable companion credential issued after the user signs in to IdeaDump in the browser and approves pairing this Android installation. The phone stores proof and credential encrypted; the server stores only hashes.
...
- **Sensitive data:** Encrypt transport and the local Finance queue; exclude unrelated notifications, OTPs, credentials, and full financial notification bodies from diagnostics. Exclude local data from cloud backup and device transfer. Delete raw server text atomically on confirmation, rejection, or duplicate resolution; retain the structured record and replay digest.
...
Source implementation is available in this repository. Build, rollout, and test evidence are maintained in [Android companion integration](../ANDROID_COMPANION.md) and [the native README](../../android/README.md). Two-field car presentation and end-to-end live notification acceptance remain required.

- **Migrations:** Three additive forward migrations add companion pairing, Finance notification intake, and pairing attempt limits. Apply the canonical files listed in the rollout guide; do not rewrite applied migrations.
- **Environment:** Use the existing server-only Supabase credentials and public HTTPS `APP_ORIGIN`. The Android Gradle property `companionOrigin` must match. The APK contains no server secret.
- **Deployment order:** Prepare compatible backend and Finance Review code with the three forward migrations before enabling pairing and ingestion. Sideload the APK for Android 13/16, live Ryt/TNG, and head-unit acceptance. Validate Spotify continuity and both lyric fields before declaring the MVP accepted. UOB enablement follows its separate sample validation.
...
Forward migrations provide:

- companion device records;
- notification event lineage;
- notification intake and ledger lineage, with manual-review enforcement and atomic raw-text deletion.
...
- **Automated:** Run the Android and backend scenarios below. For future application changes, complete `npm audit`, `npm audit --omit=dev`, `npm run lint`, `npx tsc --noEmit`, `npm test`, and `npm run build` on the repository runtime. If shared OCR code changes, also run the Finance OCR audit, typecheck, test, and build checks from `services/finance-ocr/`. Android build and connected-device commands are documented in `android/README.md`.
- **Browser:** Verify `/finance/review` confirmation, edits, rejection, duplicate handling, and missing-field recovery on mobile and desktop. Check `/finance/transactions`, `/finance/settings`, `/finance/add`, companion-to-PWA navigation, access denial, and existing manual, screenshot, and PWA share flows. Verify `/companion/pair` and Finance settings, Companion, including the login return path.
...
- **Queue:** Persist before upload, retain identifiers across retries, pause capture and upload when disabled, resume interrupted work, and remove local payloads after durable acceptance. Pause new capture with a visible error at 1,000 queued events while existing uploads continue, instead of silently deleting data. Queue ownership never changes across account switches.
