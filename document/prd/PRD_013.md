...
| Topic | Decision |
|---|---|
| Android versions | Support and verify the companion on Android 13 and Android 16. Other versions and the minimum SDK declaration are not yet specified. |
| Initial Finance apps | Ryt, TNG, and UOB. Verify the installed package identities and representative notification formats before defining parser fixtures. |
...
All recommendations in this section are proposals from the supplied draft or planning notes. They are not confirmed decisions. Answers can refine this PRD without changing its Draft status.

### 1. Pairing credential and callback design

**Owner:** Ashley, with implementation input from Engineering.

**Confirmed choice:** Browser-based device pairing. The user opens IdeaDump in the browser, signs in, and approves linking this Android installation to the account. The app receives its own scoped credential for permitted companion operations and can upload while the browser is closed. The user can revoke that device independently; Lyrics does not require pairing.

**Remaining decision:** Design the pairing code or callback exchange, credential scopes, expiry and refresh, secure Android storage, device revocation, and recovery from expired or revoked credentials. Pairing must bind the correct signed-in account to the intended Android installation. The exact token format and endpoint contracts are not yet selected.

### 2. Notification-to-source mapping

**Owner:** Ashley, with implementation input from Engineering.

**Confirmed scope:** One account per app for Ryt, TNG, and UOB, mapping each selected Android package to one user-owned Finance source. The supplied draft's multi-account option is outside the MVP.

**Remaining decision:** Define where the user configures this mapping and how missing, archived, deleted, or changed Finance sources are handled for queued events. Package identity remains separate from the editable source label, and candidates still require review.

### 3. Unsupported notification handling

**Owner:** Ashley, with implementation input from Engineering.

Options:

- silently ignore events that do not resemble financial transactions;
- store a bounded failure record for troubleshooting.

**Recommendation:** discard clearly irrelevant events immediately, retain only sanitized diagnostic status for ambiguous configured-source events.

### 4. PWA and browser routing

**Owner:** Ashley, with implementation input from Engineering.

**Confirmed choice:** Open the existing IdeaDump PWA or system browser. An embedded WebView is outside the MVP.

**Remaining decision:** Validate installed-PWA routing, browser fallback, sign-in return behavior, and direct navigation to Finance Review. These UI actions must not be required to keep notification capture or Lyrics running.

### 5. Target device and Android Auto compatibility

**Owner:** Ashley and Engineering.

Android 13 and Android 16 are confirmed targets. The phone models, Android Auto version, car/head unit, and wired or wireless connection remain unidentified; the minimum SDK declaration is also undecided. The draft requires Spotify audio continuity while the companion is selected and two independently updated lyric fields; neither behavior has been validated for a named target. Record the environment during the feasibility test and validate both behaviors on that hardware before committing to the car-display implementation. If two fields are unavailable, single-line mode remains a compatibility fallback and does not meet AC-004 unless Ashley explicitly changes the requirement.

### 6. Initial Finance applications and representative formats

**Owner:** Engineering validates formats for the apps selected by Ashley.

Ryt, TNG, and UOB are confirmed, with one account per app. Verify their installed Android package identities and use sanitized representative notifications to define supported transaction, balance, promotion, OTP, redacted, and ambiguous cases. Resolve how to select a transaction date and time zone when the notification omits them; capture time must not silently be assumed to be the transaction date.

### 7. Android distribution and signing

**Owner:** Ashley.

The Android companion will be a subproject in this repository alongside the existing Next.js backend. Specify the APK installation/update channel and signing ownership. Public Play Store release remains outside the MVP requirement.

### 8. Notification identity and queue recovery

**Owner:** Engineering, with Ashley deciding visible behavior.

Define how repeated listener callbacks, updates to the same Android notification key, grouped notifications, and reposts map to captured events. The existing `client_event_id` requirement prevents upload retries from duplicating one event; it does not yet define whether two callbacks represent that same event. Options include a stable event plus revision handling or separate events for meaningful content changes, with Finance duplicate assessment still authoritative.

Define the server acknowledgement that permits local retirement, recovery after process death during `uploading`, retry limits/backoff, temporary token expiry versus revoked access, and whether disabling capture, signing out, or re-pairing pauses or clears previously queued data. Pending events must never be reassigned to another account.

### 9. Privacy filtering, retention, and storage limits

**Owner:** Ashley and Engineering.

Package selection alone does not distinguish transactions from OTPs or personal messages emitted by the same enabled app. Reconcile durable capture with the stated prohibition on retaining unrelated content by defining what is discarded or sanitized before persistence, what reaches review, and what produces only a safe diagnostic code. The unsupported-event recommendation in decision 3 remains unconfirmed.

Set maximum field/payload sizes, local queue age/count/size limits, server raw-text retention, expiry/deletion behavior, and encrypted-backup rules. Explain what happens to accepted, unsupported, terminal-failure, and revoked-device events. Preserve idempotency for the agreed replay window without retaining unnecessary notification text indefinitely.

### 10. Finance lineage and correction learning

**Owner:** Engineering, with Ashley confirming review behavior.

Choose whether the generic intake model is extended or backed by a dedicated notification event table, how confirmation preserves notification origin, and which correction/rule-learning paths are semantically valid for notification text. The current ledger source contract and OCR-oriented review evidence require explicit treatment, as described under Data and Interfaces.

### 11. Lyric matching and cache behavior

**Owner:** Engineering, with Ashley confirming phone settings behavior.

Define match confidence, duration tolerance, cache expiry and limits, local LRC association/override behavior, and timing-offset bounds. Preserve FR-L009 for a manually confirmed local override. The supplied draft refers to a LyricBridge Android Auto PRD that was not supplied or found in this repository; do not infer extra requirements from that unavailable reference.
