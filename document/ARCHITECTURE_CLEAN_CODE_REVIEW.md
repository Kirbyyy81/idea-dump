...
| AC-014 | Monolithic OpenAPI definition | **Done** | The documented Logs, Projects/Ingest, Tickets, Film, and Documentation paths live in separate `lib/openapi/` domain modules. `index.ts` composes the final specification, and `npm run test:openapi` verifies its path and composition contract. | 2026-09-23 |
...
| AC-016 | Overloaded application shell | **Partial** | Canonical module route mapping in client-safe `lib/rbac/routes.ts` now includes Documentation and is used by `AppShell` authorization and Sidebar navigation activity. Documentation API routes authorize independently through the shared module guard. `AppShell` still owns project loading, navigation, responsive behavior, spacing, and loading UI. Done when protected layout, page container, loading state, and mobile navigation responsibilities are explicit. | 2026-09-23 |
