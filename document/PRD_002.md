...
| Update human-created logs | Yes   | No                         |
| Delete logs               | Yes   | No                         |

API-key agents cannot overwrite human-created logs.
...
* Agent may update entries with `source=agent` only
...
* Agent cannot delete
* Agent cannot overwrite human rows
