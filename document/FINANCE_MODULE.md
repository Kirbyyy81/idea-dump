...
Migration `20260915095835_finance_budget_current_settings.sql` passed the disposable-database upgrade fixture and lifecycle/concurrency suite. The linked-project deployment dry run lists only this migration; unrelated pending OCR migrations are excluded. Live application is pending explicit backup/deployment approval. The compatibility adapter keeps the application usable with the current schema in the meantime. No release version files were changed, and this work uses `fix:` commits within release 15.
