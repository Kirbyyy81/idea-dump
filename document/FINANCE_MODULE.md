...
The isolated CLI deployment preview listed only `20260915165526_finance_budget_filters_and_cycle_totals.sql`, excluding unrelated pending migrations. Its repository and deployment-copy SHA-256 both equal `F0BE38DEB1525F5BAC449F9F848D0B2C6AE334953B748BE233E99ACE1057A283`. Live deployment is pending: automatic approval review rejected exporting the five private budgeting tables through the Supabase connector before local encryption. No fresh backup was exported and no live schema change was applied. Obtain explicit approval for that export, save and verify the encrypted backup, then apply the reviewed migration and verify live table counts, cycle preservation, Data API reads, grants/RLS and cron. The existing live five-table schema remains compatible with the simplified application.
