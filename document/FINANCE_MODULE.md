...
`finance_budgets` holds owner, stable identity, current settings, optimistic revision, creation request UUID and lifecycle. `finance_budget_sources` and `finance_budget_categories` link selections directly to a budget through tenant-safe relationships, retaining original IDs and labels when archived-budget references are deleted. There is no configuration-version table. Cycles have one open row per budget. Completed and partial cycles retain an immutable `configuration_snapshot` alongside frozen totals; `finance_budget_cycle_breakdowns` stores their frozen source/category totals and labels. Open cycles read current settings, and configuration edits recalculate them. Closed cycles read their own settings, including their original amount, without consulting the current budget. They contain no transaction snapshots.
