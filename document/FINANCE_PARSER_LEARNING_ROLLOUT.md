...
## Legacy learning retirement

`20260915172224_retire_legacy_finance_learning.sql` disables legacy category/direction rules (`finance_rules.source='learning'`) and reference transforms, including previously accepted suggestions. Their rows and evidence remain. New inserts, reactivation and changing a rule's origin are blocked. Pending suggestions remain historical pending rows, are omitted from API responses, and cannot be activated. The legacy entry point returns zero. Existing manual rules and algorithm 2/3 learning continue; no replacement category learner is introduced.

The scheduled refresh retains its signature, invocation replay, advisory lock, safe failure reporting and 90-day retention. New runs report zero legacy activity; retries of historical invocations return the original stored result. New parsing baselines exclude legacy transforms, while stored candidates and evidence are not rewritten. Run explicit operator promotion only after the existing evidence checks succeed; retirement does not promote or reset templates.

For a separately authorized production rollout:

1. Reconcile the live migration ledger with installed definitions and review the CLI dry run. Do not replay historical migrations merely because a ledger entry is missing.
2. Pause new intake/retry processing and let in-flight OCR attempts finish before cutover. Apply the retirement migration before deploying the cleaned-up application and OCR service. The migration shares the learner's advisory lock.
3. Deploy both validated runtimes, resume processing, and confirm manual rules, receipt review, algorithm 2/3 learning status and shadow rules still work. Legacy entries should display as retired, with no activation controls; authorized suggestion mutations return 410.
4. Inspect the next scheduled learning run for successful parser refresh and zero legacy activity. Keep existing cron schedules and feature flags unchanged. Do not reprocess uploads or alter confirmed transactions.

Verification: on an isolated database with the adopted schema and every forward migration preceding retirement, run `supabase/tests/finance_legacy_retirement_upgrade.test.sql`. It applies the exact retirement migration inside a rollback-only transaction and checks existing manual/legacy rows, history, stale activation, ownership, and invocation retries. Then apply retirement locally and run all current parser, receipt-format, finalisation and budget SQL suites. The v2 timeout test now injects a delay into the active v2 learner, and the cutoff suite asserts that legacy generation is never invoked. Run application/OCR checks and PostgreSQL parity as usual.

Retirement validation (2026-09-16): 335 application tests and 453 OCR tests passed, including 131 PostgreSQL parity cases and the targeted context-loading test. The exact upgrade migration test, nine post-retirement SQL suites (including budgets), and 18 direct SQL security assertions passed. Application lint, both type checks, both production builds, and all four dependency audits passed with zero vulnerabilities. Local PostgreSQL used hosted-prerequisite scaffolding; pgTAP assertions were run as equivalent direct SQL checks. No production migration, cron run, deployment, flag change, or historical reprocessing was performed.
